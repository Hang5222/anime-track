...
*   **返回顶部按钮与虚拟列表冲突**：点击返回顶部按钮时，页面只会向上滚动一部分而不是直接回到顶部。原因是 `window.scrollTo({ behavior: 'smooth' })` 的平滑滚动动画与虚拟列表的滚动监听冲突，虚拟列表在滚动过程中检测到位置变化会触发重新计算和渲染，干扰了滚动动画。解决方案是将虚拟列表自带的的 `scrollToOffset` 方法通过 props 传递给 BackToTop 组件，由虚拟列表内部处理滚动动画，避免冲突。
